(function(){
  ...
  // `TASK-2026-00131` -- which is what this screen printed, exports included, and
  // a payment sheet naming ids is no use to whoever signs it. wm_dashboard's
  // task_names action returns {docname: subject}; the fallback is the docname.
  //
  // Grouping keys are deliberately NOT resolved: they key on the docname because
  // that is the identity, and two distinct tasks may share a subject.
  var TASK_NAMES={};
  ...
  function call(args, isWrite, method){
    // `method` names another screen's script -- only wm_dashboard, for its
    // task_names map. It is the third argument rather than the second because
    // the second already means isWrite here, unlike the other screens.
    var p=new URLSearchParams();
    for(var k in args){ if(args[k]!=null) p.append(k, args[k]); }
    var ep = method ? ("/api/method/"+method) : API;
  ...
  function loadInsights(){
    ...
    call(args).then(function(d){
      ...
      var sc=el("ins-scope");
      if(sc) sc.textContent=((d.window&&d.window.farm)||("All "+TX("top_plural","Farms").toLowerCase()))+" · "+((d.window&&d.window.from)||"")+" → "+((d.window&&d.window.to)||"");
      renderInsights(d, b);
  ...
  function renderInsights(d, b){
    ...
    h+='<div style="display:flex;gap:10px;align-items:center;margin-bottom:8px;flex-wrap:wrap">'+
       '<input type="text" id="ins-q" placeholder="'+esc("Filter by name, "+TX("top_singular","Farm").toLowerCase()+" or activity…")+'" style="max-width:320px">'+
  ...
  function insDefs(){
    ...
    return [
      {key:"avail", label:"Available", rows:d.available_list||[],
       ...
       cols:[["Worker","nm",0],["ID","emp",0],[TX("top_singular","Farm"),"farm",0],["Designation","designation",0]]},
      {key:"asgd", label:"Assigned", rows:(d.assigned_list||[]).map(function(r){ r.period=(r.latest_from||"")+(r.latest_to?(" → "+r.latest_to):""); return r; }),
       ...
       cols:[["Worker","nm",0],[TX("top_singular","Farm"),"farm",0],["Assignments","assignments",1],["Tasks","task_count",1],["Task detail","task_list",0],["Latest period","period",0]]},
      {key:"owed", label:"Owed money", rows:d.unpaid_list||[],
       ...
       cols:[["Worker","nm",0],[TX("top_singular","Farm"),"farm",0],["Days","days",1],["Owed (KES)","owed",1],["Earliest unpaid","oldest",0],["Latest","newest",0]]},
      {key:"idle", label:"Not working", rows:d.idle_list||[],
       ...
       cols:[["Worker","nm",0],[TX("top_singular","Farm"),"farm",0],["Live assignments","assignments",1]]},
      {key:"tasks", label:"Activities", rows:(d.task_costs||[]).map(function(r){
          r.cpu=(r.qty>0)?(r.pay/r.qty):null; return r; }),
       note:"Confirmed labour spend per activity per "+TX("top_singular","Farm").toLowerCase()+". KES/unit is what each unit of output actually cost — the column that exposes expensive work even when the total looks small.",
       search:["label","farm"], sort:{c:"pay",dir:-1},
       cols:[["Activity","label",0],[TX("top_singular","Farm"),"farm",0],["Spend (KES)","pay",1],["Share","share",1],["KES/unit","cpu",1],["Units","qty",1],["People","workers",1],["Mandays","mandays",1]]},
      {key:"top", label:"Top earners", rows:d.top_workers||[],
       ...
       cols:[["Worker","nm",0],[TX("top_singular","Farm"),"farm",0],["Days","days",1],["Output","qty",1],["Earned (KES)","pay",1]]}
    ];
  }
  function drawInsTab(){
    var defs=insDefs();
  ...
  function renderFarmChips(){
    ...
    var h='<div class="fchip allchip'+(Object.keys(ST.activeFarms).length?"":" on")+'" data-farm="">All '+esc(TX("top_plural","Farms")).toLowerCase()+'</div>';
    ...
    call({ action:"pay_worker_detail", employee:emp, from_date:r.from, to_date:r.to })
      .then(function(d){
        ...
        var h='<div class="tablewrap"><div class="tablescroll"><table><thead><tr>'+
          '<th>Date</th><th>Task</th><th>'+esc(TX("top_singular","Farm"))+'</th><th class="n">Qty</th><th class="n">Rate</th><th class="n">Amount</th></tr></thead><tbody>';
        jobs.forEach(function(j){
          h+='<tr><td>'+esc(j.wdate||"")+'</td><td>'+esc(taskName(j.task))+'</td><td>'+esc(lbl(j.farm)||"")+'</td>'+
  ...
  function accFilterBar(){
    ...
    return '<div class="filters" style="margin-bottom:10px">'+
      ...
      (chips?'</div><div class="farmchips" style="margin-bottom:10px">'+
        '<div class="fchip allchip'+(Object.keys(ST.accFarms||{}).length?"":" on")+'" data-accfarm="">All '+esc(TX("top_plural","Farms")).toLowerCase()+'</div>'+chips+'</div>'
       :'</div>');
  }
  ...
  function renderAccounts(){
    ...
    if(!ST.accTotal){
      box.innerHTML=accFilterBar()+'<div class="empty"><b>Nothing matches these filters</b>'+
        fmt(ST.accQueueTotal)+' run'+(ST.accQueueTotal===1?'':'s')+' in the queue overall &mdash; widen the dates, clear the '+esc(TX("top_singular","Farm")).toLowerCase()+' or the search.</div>';
      wireAccFilters(box);
      return;
    }
    var h=accFilterBar();
  ...
  function renderAuditChips(){
    ...
    var h='<div class="fchip allchip'+(Object.keys(AU.farms).length?"":" on")+'" data-farm="">All '+esc(TX("top_plural","Farms")).toLowerCase()+'</div>';
  ...
  function discTable(c){
    ...
    if(c.key==="multi_farm_day"){
      h+='<th>Worker</th><th>Day</th><th>'+esc(TX("top_plural","Farms"))+'</th><th class="n">KES</th><th>Actuals docs</th>';
      rows.forEach(function(r){
      ...
      h+='<th>Actuals doc</th><th>'+esc(TX("top_singular","Farm"))+'</th><th>Task</th><th>Entered by</th><th>HR appr.</th><th>GM appr.</th><th class="n">KES</th>';
      rows.forEach(function(r){
        body+='<tr><td class="m">'+esc(r.actuals)+'</td><td>'+esc(r.farm||"")+'</td><td>'+esc(taskName(r.task))+'</td>'+
      ...
      h+='<th>Worker</th><th>Day worked</th><th>Left on</th><th>'+esc(TX("top_singular","Farm"))+'</th><th>Task</th><th class="n">KES</th><th>Doc</th>';
      rows.forEach(function(r){
        body+='<tr><td>'+wlink(r)+'</td><td class="m">'+esc(dshort(r.wdate))+'</td><td class="m" style="color:var(--bad)">'+esc(dshort(r.left_date))+'</td>'+
          '<td>'+esc(r.farm||"")+'</td><td>'+esc(taskName(r.task))+'</td><td class="n m">'+fmt(r.amount,2)+'</td><td class="m" style="font-size:10px">'+esc(r.actuals)+'</td></tr>';
      });
      ...
      h+='<th>Worker</th><th>HR status</th><th>'+esc(TX("top_singular","Farm"))+'</th><th>Task</th><th>Assignment</th><th>Period</th>';
      rows.forEach(function(r){
      ...
      h+='<th>Worker</th><th>Day</th><th>'+esc(TX("top_singular","Farm"))+'</th><th>Task</th><th class="n">Copies</th><th class="n">Total KES</th><th class="n">Excess KES</th><th>Docs</th><th>Entered by</th>';
      rows.forEach(function(r){
        body+='<tr><td>'+wlink(r)+'</td><td class="m">'+esc(dshort(r.wdate))+'</td><td>'+esc(r.farm||"")+'</td><td>'+esc(taskName(r.task))+'</td>'+
      ...
      h+='<th>Worker</th><th>Day</th><th>'+esc(TX("top_singular","Farm"))+'</th><th>Task</th><th class="n">Qty</th><th class="n">KES</th><th class="c">Presence</th>'+(c.key==="leave_paid"?'<th>Leave</th>':'')+'<th class="c">Paid</th><th>Doc</th>';
      rows.forEach(function(r){
        body+='<tr><td>'+wlink(r)+'</td><td class="m">'+esc(dshort(r.wdate))+'</td><td>'+esc(r.farm||"")+'</td><td>'+esc(taskName(r.task))+'</td>'+
    ...
    if(c.count>=200) h+='<div class="note">Showing the first 200 rows — narrow the date range or '+esc(TX("top_singular","Farm")).toLowerCase()+' filter to see the rest.</div>';
    return h;
  }
  ...
  function renderAuditSummary(box){
    ...
    var h='<div class="tablewrap"><div class="tablescroll"><table id="au-table"><thead><tr>'+
      '<th>'+esc(TX("top_singular","Farm"))+'</th><th>Task</th><th>'+esc(TX("unit_singular","Block"))+'</th><th>Assignment</th>'+
    ...
    rows.forEach(function(s){
      h+='<tr>'+
        '<td>'+esc(s.farm)+'</td><td>'+esc(taskName(s.task))+'</td><td>'+esc(s.block||"—")+'</td>'+
  ...
  function renderAuditDetail(box){
    ...
    var h='<div class="tablewrap"><div class="tablescroll"><table id="au-table"><thead><tr>'+
      '<th>'+esc(TX("top_singular","Farm"))+'</th><th>Task</th><th>Assignment</th><th>Worker</th><th>ID</th><th>Type</th>'+
      '<th>Date</th><th class="n">Qty</th><th class="n">Amount KES</th><th class="c">Paid</th><th>Run</th></tr></thead><tbody>';
    rows.forEach(function(r){
      h+='<tr>'+
        '<td>'+esc(r.farm)+'</td><td>'+esc(taskName(r.task))+'</td><td>'+esc(r.assignment||"—")+'</td>'+
  ...
  function renderAuditWorkers(box){
    ...
    function paint(){
      ...
      var flt=rows.filter(function(g){
        if(!vq) return true;
        return (g.nm+" "+g.emp+" "+g.farm_list+" "+Object.keys(g.tasks).map(taskName).join(" ")).toLowerCase().indexOf(vq)>-1;
      });
  ...
  function renderWorkerHistory(d){
    ...
    h+='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px 22px;padding:12px 16px;border:1px solid var(--line);border-radius:12px;background:var(--wash);margin-bottom:14px;font-size:12px">'+
      ...
      '<div><div style="font-size:9.5px;letter-spacing:.1em;text-transform:uppercase;color:var(--mute);font-weight:600">'+esc(TX("top_singular","Farm"))+'</div><b>'+esc(lbl(info.farm)||"—")+'</b><div style="color:var(--mute)">'+esc(info.status||"")+'</div></div>'+
    ...
    if(!tasks.length){ h+='<div class="empty">No confirmed work in this window.</div>'; }
      ...
      h+='<div class="tablewrap"><table><thead><tr>'+
        '<th>Task</th><th>'+esc(TX("unit_singular","Block"))+'</th><th>Standard</th><th>Period worked</th><th class="n">Days</th><th class="n">Qty</th>'+
        '<th class="n">Rate</th><th class="n">Amount KES</th><th class="n">Unpaid</th><th class="c">Status</th></tr></thead><tbody>';
      tasks.forEach(function(t){
        h+='<tr><td><b>'+esc(taskName(t.task)||"—")+'</b></td><td>'+esc(lbl(t.block)||"—")+'</td>'+
    ...
    tasks.forEach(function(t){
      ...
      h+='<div style="border:1px solid var(--line);border-radius:14px;margin-bottom:14px;overflow:hidden">'+
        '<div style="padding:12px 16px;background:var(--wash);display:flex;flex-wrap:wrap;gap:6px 18px;align-items:baseline">'+
          '<div style="font-weight:600;font-size:13px">'+esc(taskName(t.task)||"—")+' <span style="color:var(--mute);font-weight:500">· '+esc(lbl(t.block)||"—")+' · '+esc(lbl(t.farm)||"")+'</span>'+(cardFlagged?' <span title="'+cardFlagged+' flagged day'+(cardFlagged>1?'s':'')+' — see the Discrepancies tab" style="color:#b91c1c;font-weight:700;font-size:11px">&#9873;'+cardFlagged+'</span>':'')+'</div>'+
          '<span style="flex:1"></span>'+
      ...
      rows.forEach(function(r){
        ...
        var qtyCell = r.editable
          ? '<input type="number" min="0" step="any" class="wrq" data-row="'+esc(r.rowname||"")+'" data-orig="'+(r.qty||0)+'" data-rate="'+(r.rate||0)+'" value="'+(r.qty||0)+'" style="width:86px;font-family:inherit;font-size:12px;border:1px solid var(--line);border-radius:8px;padding:4px 8px;text-align:right;background:rgba(255,255,255,.7)">'
          : fmt(r.qty);
    ...
    if(tasks.length){
      h+='<div id="wr-bulkbar" style="position:sticky;bottom:0;display:none;align-items:center;gap:12px;padding:10px 14px;background:rgba(250,250,246,.95);-webkit-backdrop-filter:blur(10px);backdrop-filter:blur(10px);border:1px solid var(--line);border-radius:12px;margin-top:6px">'+
        '<span class="hint" id="wr-chg" style="font-weight:600;color:var(--ink)"></span><span style="flex:1"></span>'+
        '<button type="button" class="btn sm" id="wr-undoall">Undo</button>'+
        '<button type="button" class="btn good sm" id="wr-saveall">Save changes</button></div>';
    }
    h+='</div>';
    ...
    if(!nIss){
      ...
      groups.forEach(function(g){
        ...
        g.rows.forEach(function(r){
          h+='<tr><td class="m">'+esc(dshort(r.wdate))+'</td><td>'+esc(taskName(r.task))+'</td>'+
            '<td class="n m">'+fmt(r.qty)+'</td><td class="n m">'+fmt(r.amount,2)+'</td>'+
            '<td class="c m">'+discPres(r)+'</td>'+
    ...
    // ── bulk day editing: type into any Qty box, one Save at the bottom ──
    function wrChanged(){
      var out=[];
      el("pd-body").querySelectorAll("input.wrq").forEach(function(inp){
        var v=parseFloat(inp.value);
        var o=parseFloat(inp.getAttribute("data-orig"));
        if(!isNaN(v) && v>=0 && Math.abs(v-o)>0.0001) out.push({row:inp.getAttribute("data-row"), qty:v});
      });
      return out;
    }
    function wrSync(){
      var ch=wrChanged();
      var bar=el("wr-bulkbar"); if(!bar) return;
      bar.style.display=ch.length?"flex":"none";
      var amt=0;
      el("pd-body").querySelectorAll("input.wrq").forEach(function(inp){
        var v=parseFloat(inp.value), o=parseFloat(inp.getAttribute("data-orig")), rt=parseFloat(inp.getAttribute("data-rate"))||0;
        var cell=el("pd-body").querySelector('[data-amt-for="'+inp.getAttribute("data-row")+'"]');
        if(cell && !isNaN(v) && Math.abs(v-o)>0.0001){ cell.innerHTML='<b style="color:#a06000">'+fmt(v*rt,2)+'</b>'; amt+=v*rt; }
        else if(cell && !isNaN(v)){ cell.textContent=fmt(v*rt,2); }
      });
      if(ch.length) el("wr-chg").textContent=fmt(ch.length)+" day"+(ch.length>1?"s":"")+" changed — pay recomputes at each row's rate";
    }
    el("pd-body").querySelectorAll("input.wrq").forEach(function(inp){ inp.oninput=wrSync; });
    var ua=el("wr-undoall");
    if(ua){ ua.onclick=function(){ el("pd-body").querySelectorAll("input.wrq").forEach(function(inp){ inp.value=inp.getAttribute("data-orig"); }); wrSync(); }; }
    var sa=el("wr-saveall");
    if(sa){
      sa.onclick=function(){
        var ch=wrChanged();
        if(!ch.length){ toast("Nothing changed","bad"); return; }
        sa.disabled=true; sa.textContent="Saving…";
        call({action:"pay_worker_edit_days", employee:info.employee,
              rows:ch.map(function(c){ return c.row+"~"+c.qty; }).join("|")}, true)
          .then(function(d){
            sa.disabled=false; sa.textContent="Save changes";
            if(d.error){ toast(d.error,"bad"); return; }
            var errs=(d.errors||[]);
            toast(fmt(d.updated)+" day"+(d.updated===1?"":"s")+" corrected"+(errs.length?(" · "+errs.length+" skipped: "+errs.slice(0,2).map(function(e){ return e.error; }).join("; ")):""), errs.length?"bad":"good");
            openWorkerReview(info.employee, WR);
            if(WR.refresh) WR.refresh();
          })
          .catch(function(e){ sa.disabled=false; sa.textContent="Save changes"; toast("Could not save: "+e.message,"bad"); });
      };
    }
    if(WR.targetTab){
  ...
  function printAudit(){
    ...
    var farms=Object.keys(AU.farms).length?Object.keys(AU.farms).join(", "):("All "+TX("top_plural","Farms").toLowerCase());
    var t=AU.totals||{};
    ...
    var sumRows=(AU.summary||[]).map(function(s){
      var a=[esc(s.farm),esc(taskName(s.task)),esc(s.block||""),esc(s.assignment||""),fmt(s.planned_people),fmt(s.assigned_count),fmt(s.actual_qty),fmt(s.workers),fmt(s.total_pay,2),fmt(s.paid_pay,2),fmt(s.unpaid_pay,2),esc(s.pay_status),esc(s.run_refs||""),esc(s.entered_by||"")];
      a._n=4; return a;
    });
    var detRows=(AU.detail||[]).map(function(d){
      var a=[esc(d.farm),esc(taskName(d.task)),esc(d.assignment||""),esc(d.emp_name||d.emp),esc(d.emp),esc(d.emp_type||""),esc(d.wdate||""),fmt(d.qty),fmt(d.amount,2),esc(d.pay_status),esc(d.run_ref||"")];
      a._n=7; return a;
    });
    ...
    var head='<h1>'+esc(orgPrefix())+'Payroll Audit</h1><div class="meta">Confirmed actuals &middot; work done '+esc(r.from)+' to '+esc(r.to)+' &middot; '+esc(farms)+
    ...
    var body='<h2>Summary — by task</h2>'+
      tbl([TX("top_singular","Farm"),"Task",TX("unit_singular","Block"),"Assignment","Planned","Assigned","Qty","Workers","Total KES","Paid KES","Unpaid KES","Status","Run","Entered by"], sumRows)+
      '<h2>Detail — by worker &amp; day</h2>'+
      tbl([TX("top_singular","Farm"),"Task","Assignment","Worker","ID","Type","Date","Qty","Amount KES","Paid","Run"], detRows);
    win.document.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>'+esc(orgPrefix())+'Payroll Audit</title><style>'+css+'</style></head><body>'+head+body+
  ...
  function buildWorkerXLSX(d){
    ...
    var s1=[
      ...
      [TX("top_singular","Farm"), info.farm||"", "Joined", info.date_of_joining||""],
    ...
    tasks.forEach(function(t){
      var kk=(t.task||"")+"|"+(t.block||"")+"|"+(t.farm||"");
      var rows=(byTask[kk]||[]).slice().sort(function(a,b){ return (a.wdate||"")<(b.wdate||"")?-1:1; });
      s2.push([(taskName(t.task)||"—")+" · "+(t.block||"—")+" · "+(t.farm||""), "", "", "KES "+fmt(t.amount,2), t.pay_status||""]);
  ...
  // The organisation name comes from Work Management Settings via the page, so
  // an audit exported from another project is not headed with someone else's.
  function orgPrefix(){
    var org=(window.WM_ORG||"").trim();
    return org?org+" \u2014 ":"";
  }

  function auditFileBase(){
    var f=(el("au-from").value||"start"), t=(el("au-to").value||"today");
    var org=(window.WM_ORG||"").replace(/[^A-Za-z0-9]+/g,"_").replace(/^_|_$/g,"");
    return (org?org+"_":"")+"Payroll_Audit_"+f+"_to_"+t;
  }

  function buildXLSX(){
    var sum=(AU.summary||[]).map(function(s){ return {
      "Farm":s.farm, Task:taskName(s.task), "Block":s.block||"", Assignment:s.assignment||"",
    ...
    var det=(AU.detail||[]).map(function(d){ return {
      "Farm":d.farm, Task:taskName(d.task), "Block":d.block||"", Assignment:d.assignment||"",
  ...
  function exportAuditCSV(){
    ...
    var sum=(AU.summary||[]).map(function(s){ return {
      "Farm":s.farm,Task:taskName(s.task),"Block":s.block||"",Assignment:s.assignment||"",Planner:s.planner_request||"",
    ...
    var det=(AU.detail||[]).map(function(d){ return {
      "Farm":d.farm,Task:taskName(d.task),"Block":d.block||"",Assignment:d.assignment||"",Worker:d.emp_name||d.emp,ID:d.emp,
    ...
    fetch("/api/method/wm_dashboard?action=payment_detail&payment="+encodeURIComponent(name),
      ...
      .then(function(j){
        ...
        if(!lines.length){
          ...
          h+='<div class="tablewrap"><div class="tablescroll"><table><thead><tr>'+
             '<th>Worker</th><th>'+esc(TX("top_singular","Farm"))+'</th><th class="n">Days</th><th class="n">Qty</th><th class="n">Amount</th></tr></thead><tbody>';
